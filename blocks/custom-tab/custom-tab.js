export default function decorate(block) {
  const header = block.querySelector('.custom-tab > div:first-child');
  const content = block.querySelector('.custom-tab > div:nth-child(2)');
...
var aTags = document.querySelectorAll('.custom-tab a');
...
// Add alt attributes to images within the custom-tab section
document.addEventListener("DOMContentLoaded", function() {
  const imageParagraphs = document.querySelectorAll('.custom-tab div:nth-child(2) p');
  imageParagraphs.forEach(paragraph => {
      const imageLinks = paragraph.querySelectorAll('a[href$=".png"], a[href$=".jpg"], a[href$=".jpeg"], a[href$=".gif"], a[href$=".svg"]');
